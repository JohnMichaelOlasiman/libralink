...
export function AdminTransactionsClient({ initialTransactions }: AdminTransactionsClientProps) {
  ...
  const mapToCardProps = (t: BorrowRequestWithDetails) => ({
    ...
    userAvatar: (t.user as any)?.avatar_url || "/placeholder.svg",
    type: t.status === "returned" ? "return" : ("borrow" as any),
    status: t.status === "approved" ? "active" : (t.status as any),
    borrowDate: t.approved_at
